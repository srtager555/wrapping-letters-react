...
function WrappLetter(_ref) {
  ...
  return function () {
    ...
    var arrElements = crumbledText.map(function (wrappElement, index) {
      ...
      let newClass = specialClass; // And here verify if is an array or a string
      ...
      if (!perWord) {
        ...
        if (!searchWordValue.some(element => index + element.length <= crumbledText.length)) return outSpecialClass();
        if (!searchWordValue.some(element => crumbledText.slice(index, index + element.length).join("") === element)) return outSpecialClass();
