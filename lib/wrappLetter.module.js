...
function WrappLetter(_ref) {
  ...
  return function () {
    ...
    var arrElements = crumbledText.map(function (wrappElement, index) {
      if (!perWord && searchWordValue.length > 0 && wrappElement[0] === searchWordValue[0] && index + searchWordValueLength <= crumbledText.length && crumbledText.slice(index, index + searchWordValueLength).join("") === searchWordValue.join("")) {
