...
exports.WrappingLetters = WrappingLetters;
...
function WrappLetter(_ref, text, ClassToAdd) {
  ...
  var wrapLetters = text.map((letter, index) => {
    if (searchWordValueLength !== 0 && letter === searchWordValue[0]) {
      if (index + searchWordValueLength <= text.length && text.slice(index, index + searchWordValueLength).join("") === searchWordValue.join("")) {
...
function errorFilterSpecialClass(wordOptions) {
  const searchWord = Object.keys(wordOptions[0]["SelectClass"]).filter(key => key === "wordToSearch");
  const searchWordClass = Object.keys(wordOptions[0]["SelectClass"]).filter(key => key === "classToAdd");
  const searchSBW = Object.keys(wordOptions[0]["SelectClass"]).filter(key => key === "spaceBetweenWord");

  if (searchWord.length === 0 || searchWordClass.length === 0 || searchSBW.length === 0) {
    throw new Error("SelectClass must have 3 keys : wordToSearch, spaceBetweenWord and classToAdd");
  }

  if (Object.prototype.toString.call(wordOptions[0]["SelectClass"].wordToSearch) !== "[object String]" || Object.prototype.toString.call(wordOptions[0]["SelectClass"].spaceBetweenWord) !== "[object Boolean]" || Object.prototype.toString.call(wordOptions[0]["SelectClass"].classToAdd) !== "[object String]") {
...
function WrappingLetters(_ref2) {
  ...
  if (wordOptions.length > 0 && Object.prototype.toString.call(wordOptions) !== "[object Array]") {
    ...
    if (Object.keys(wordOptions[0]).length === 1 && Object.keys(wordOptions[0])[0] === "ClassToAdd") {
      if (Object.prototype.toString.call(wordOptions[0]["ClassToAdd"]) !== "[object String]") {
        throw new Error("ClassToAdd must be a string");
      }

      return WrappLetter({}, text, wordOptions[0]["ClassToAdd"]);
    } else if (Object.keys(wordOptions[0]).length === 1 && Object.keys(wordOptions[0])[0] === "SelectClass") {
      return selectSpecialClass(wordOptions, text, "");
    } else if (Object.keys(wordOptions[0]).length === 2) {
      if (Object.keys(wordOptions[0]) !== ["ClassToAdd", "SelectClass"] || Object.keys(wordOptions[0]) !== ["SelectClass", "ClassToAdd"]) {
        return selectSpecialClass(wordOptions, text, wordOptions[0]["ClassToAdd"]);
      }
    ...
    if (Object.keys(wordOptions[0]["SelectClass"]).length === 0) {
      console.warn("wordOptions is empty, return a simple wrapp of letters"); // ---- wordOptions, normal wrapp ---- //
