...
exports.outSpecialClass = outSpecialClass;

var _specialArray = require("./specialArray.process");

var _specialWordPerLettersProcess = require("./specialWordPerLetters.process.js");

function outSpecialClass(_ref) {
  let {
    wrappElement,
    newClass,
    ClassToAdd,
  ...
  let customWrapp = undefined; //here is wrapped per word.
  ...
  if (wrappToAdd !== undefined && wordToWrapp !== new String()) {
    const arrComprobations = [// basicly is the same comprobations in the index of this
    // folder
    wordToWrapp.length > 0, wordToWrapp.some(element => wrappElement === element[0]), wordToWrapp.some(element => index + element.length <= crumbledText.length), wordToWrapp.some(element => crumbledText.slice(index, index + element.length).join("") === element)];
    if (arrComprobations.every(currentValue => currentValue === true)) return (0, _specialWordPerLettersProcess.__specialWorld_PerLetters__)({
      index,
