...
function __comprobation_correct_attributes__(element) {
  const ELEMENT_KEYS = Object.keys(element); // If SelectClass an empty Object the filter ends here

  if (ELEMENT_KEYS.length === 0) throw new Error('SpecialWrapp - The Wrapper can process empty objects on strutureToAdd from SpecialWrapp "{}"');
